...
%Result Matrix
Results = zeros(size(ppc,2)*size(ka,2)*epochs,7);
r=0; 
            ...
            for i=1:10
                ...
                for e = 1:epochs 
                    ...
                    for iter=1:trpats
                        ...
                        %2x1 matrix instead of a 1x1
                        winner_s = find(Z==Z_sorted(1));                        
                        if size(winner_s,1)>1
                            winner = winner_s(1); %winner=index of winner in Z
                            losser = winner_s(2); %losser=index of losser in Z
                        else
                            winner = winner_s;
                            losser = find(Z==Z_sorted(2));
                            losser = losser(1);
                        end

                        %CHECK CONDITIONS FOR LEARNING OCCURANCE
                        %1.Two closest prototypes to pattern must be in
                        %different classes
                        %2.One of the 2 prototypes must be in the same
                        %class as the pattern
                        %3.The pattern must fall inside a small
                        %symmetric window around the midpoint of the 2
                        %prototypes
                        
                        if (L(winner)~=L(losser)) && (trlbl(iter)==L(winner) || trlbl(iter)==L(losser))...
                                && (min(Z(winner)/Z(losser),Z(losser)/Z(winner))>1-s) && (max(Z(winner)/Z(losser), Z(losser)/Z(winner))<1+s)

                            %Weight update equations
                            if trlbl(iter) == L(winner)
                                W(losser,:) = W(losser,:)-a*(transpose(Ptr(:,losser))-W(losser,:));
                                W(winner,:) = W(winner,:)+a*(transpose(Ptr(:,winner))-W(winner,:));
                            else
                                W(winner,:) = W(winner,:)-a*(transpose(Ptr(:,winner))-W(winner,:));
                                W(losser,:) = W(losser,:)+a*(transpose(Ptr(:,losser))-W(losser,:));
                            end 
...
%         filename = ['LVQ2.1. Results ', data_filename, '.xlsx'];
